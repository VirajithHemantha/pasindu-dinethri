...
export default function App() {
  ...
  // Read query params for personalized invite
  const queryParams = isClient ? new URLSearchParams(window.location.search) : new URLSearchParams();
  const guestPrefix = queryParams.get('prefix');
  const guestName = queryParams.get('name');
  ...
  const weddingDate = new Date("2026-07-16T08:00:00");
  ...
  return (
    <div className="font-sans text-stone-800 bg-brand-ivory overflow-hidden selection:bg-brand-beige-deep/20">
      ...
      <footer className="py-12 bg-white border-t border-brand-beige/20 text-center relative overflow-hidden mt-10">
        ...
        <p className="font-display text-4xl sm:text-5xl text-stone-800 mb-2">Dinethri & Pasindu</p>
